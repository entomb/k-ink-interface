    ...
    kink.result = function(param,context){
        ...
        if(!param){ //revent fuckups
            ...
            if( param.charAt(0) === "<" &&
                ...
                param.length >= 3 ){
                //HTML strings
                rArray = kink.parseHTML(param);
            }else if(context!==null){ //null context means DONT use the css selector for strings
                try{//query selector
        ...
        //InkArray.each(InkArray.convert(rArray),function(item){
        //    result.push(item);
        //});
